...
## X収益化プログラムの現況（2026-08時点）

- 現行の「Creator Revenue Sharing（旧Ads Revenue Sharing）」は**2026-09-07で終了**し、
  9月8日から新プログラム「Original Content Rewards」へ移行する（X公式発表、
  2026-08-08）。参加条件はX Premium加入・検証済みフォロワー500人以上・直近90日で
  Verified Home Timelineインプレッション50万以上。「オリジナルコンテンツ」（独自の
  報道・分析、自作の写真/動画/画像）であることが新たに求められる。
- `@RayLab`は着手前（フォロワー1）のため、**新旧いずれの制度にも当面該当しない**。
  X収益化プログラムは補助的な位置づけであり単独の柱にしない、という
  [[x-account-monetization-plan-2026-08]]の既存方針を変更する必要はない。
- 収益化算定基準は2026年1月16日付で「エンゲージメント」から「検証済みPremiumユーザー
  によるHome Timeline閲覧回数」へ変更されている（X公式発表として裏付け済み）。
  返信等のエンゲージメントは直接の収益要因ではない。
- Xの新ランキングアルゴリズム（`xai-org/x-algorithm`、2026-08-13公開）から、
  同一著者の連続表示を抑制する仕組み（repeated-author decay）と、新規著者への
  優遇（new-author boost）が確認できた。具体的な重み係数は未確認。
- 反応表現・抽選表現（「いいね/RT/リプライして」「抽選で」等）はエンゲージメント率を
  強く押し上げる要因として学術研究で確認されているが、景品企画・RT稼ぎ的な手法で
  あり、専門性ベースの一次体験発信というポジショニングと矛盾するため**意図的に
  不採用**とする（ボスの明示判断、2026-08-16）。
- 詳細調査はexocortex `wiki/concepts/x-revenue-program-and-algorithm-2026-08.md`を
  参照する。制度・算定基準は変更が速いため、実際に収益化プログラムへの参加を
  検討する段階では必ず最新情報を確認すること。
