...
- Claude Codeに設定済みの読み取り専用MCP（`mcp__xapi__*`）とは別物。読み取り専用MCPは
  ...
  （2026-08時点で実機確認済み。詳細な調査記録は旧Vaultにあり、現在は参照できない）。
...
- 収益化プログラムの制度・算定基準についての詳細調査は旧Vaultにあり、現在は
  参照できない。制度・算定基準は変更が速いため、参加を検討する段階では
  researchで最新情報を取り直し、Knowledge Vaultへ記録し直すこと。
...
- 詳細な調査経緯は旧Vaultにあり、現在は参照できない。必要になった時点で
  researchで取り直す。
